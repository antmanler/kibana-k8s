...
# of Elasticsearch to be used by Kibana.
# Both of these may be overriden if required. The two scenarios are:
...
#       KIBANA_SERVICE/elasticsearch/XXX
#    are proxied to:
...
wget -O- http://${PROXY_HOST}:${PROXY_PORT}

# Create a config.hs that defines the Elasticsearch server to be
# at /elasticsearch from the perspective of
...
function (Settings) {
  ...
  return new Settings({
     ...
     *
     */
    elasticsearch: prefix + "/elasticsearch",
